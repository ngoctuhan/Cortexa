"""
Test Case: TC-V06
Name: Query with no semantic match
Category: Validation / Input
Input/Setup: 100 messages about software engineering and programming languages embedded;
             query is completely unrelated ("ancient Egyptian pharaohs and pyramid construction").
Expected Result: semantic_messages is empty OR all returned chunk cosine_sim scores < 0.5.
"""
...
UNRELATED_QUERY = (
    "ancient Egyptian pharaohs and pyramid construction methods in the Old Kingdom period"
)
...
def run_test():
    ...
    print("Setting up 100 messages for TC-V06...")
    ...
    # Wait for embeddings using a related query (guaranteed to match) just to confirm
    # the embedder has run, then switch to the unrelated query for the actual assertion.
    anchor_query = "Tell me about Python programming language features and ecosystem"
    poll_for_embeddings(tenant_id, user_id, session_id, anchor_query)

    # Now query with the unrelated topic
    print(f"Querying with unrelated topic: {UNRELATED_QUERY[:60]!r}...")
    success, resp, _ = APIClient.get_context(
        tenant_id, user_id, session_id, UNRELATED_QUERY,
    )
    ...
    print(f"Chunks returned for unrelated query: {len(chunks)}")

    # Either no results, or all cosine similarities are below 0.5
    HIGH_SIM_THRESHOLD = 0.5
    high_sim_chunks = [c for c in chunks if c.get("cosine_sim", 0) >= HIGH_SIM_THRESHOLD]
    assert len(high_sim_chunks) == 0, (
        f"Expected no high-similarity results for unrelated query, "
        f"but got {len(high_sim_chunks)} chunk(s) with cosine_sim ≥ {HIGH_SIM_THRESHOLD}: "
        + str([{"content": c["content"][:60], "cosine_sim": c["cosine_sim"]} for c in high_sim_chunks])
    )

    if chunks:
        print(f"  Top chunk: cosine_sim={chunks[0].get('cosine_sim', 0):.4f} (below threshold {HIGH_SIM_THRESHOLD})")
    else:
        print("  No chunks returned (expected for unrelated query)")

    print("PASS: no high-similarity results for semantically unrelated query")


if __name__ == "__main__":
    run_test_wrapper("TC-V06", "Query with no semantic match", run_test)
